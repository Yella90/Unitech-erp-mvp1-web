...
exports.cancelPaiement = async (req, res) => {
  try {
    ...
    if (isInscription && !['directeur', 'promoteur'].includes(role) && !isSuperAdminRole(role)) {
      return res.status(403).json({ error: "Seul le directeur ou le promoteur peut annuler un paiement d'inscription" });
    }
  ...
  try {
    ...
    const rows = await all(
      `SELECT em.id, em.jour, em.heure_debut, em.heure_fin,
       ...
       LEFT JOIN classes c ON CAST(c.id AS TEXT) = CAST(a.classe_id AS TEXT)
       LEFT JOIN enseignants e ON CAST(e.id AS TEXT) = CAST(a.enseignant_id AS TEXT)
